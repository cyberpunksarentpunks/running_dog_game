...
drop_size = (90, 90)
drop_icon = pygame.transform.scale(
    pygame.transform.rotate(drop_origin_img, 180), drop_size
)
dropX = 0
dropY = 650
dropX_change = 0
...
def bottle_collision(bottleX, bottleY, playerX, playerY):
    distance = math.sqrt(
        math.pow(bottleX - playerX, 2) + (math.pow(bottleY - playerY, 2))
    )
    if distance < 65:
        return True
    else:
